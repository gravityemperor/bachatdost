<?php 
add_action( 'wp_enqueue_scripts', 'blogling_enqueue_styles' );
function blogling_enqueue_styles() {
	wp_enqueue_style( 'blogling-parent-style', get_template_directory_uri() . '/style.css' ); 
} 



function blogling_google_fonts() {
	wp_enqueue_style( 'blogling-google-fonts', '//fonts.googleapis.com/css2?family=Nunito:ital,wght@0,400;0,600;1,400;1,600&display=swap"', false ); 
}
add_action( 'wp_enqueue_scripts', 'blogling_google_fonts' );



function blogling_customize_register( $wp_customize ) {
	/* New Section */
	$wp_customize->add_section( 'blogling_settings', array(
		'title'      => __('Blogling Settings','blogling'),
		'priority'   => 1,
	...
	$wp_customize->add_setting( 'show_post_categories', array(
		'default' => 0,
		'sanitize_callback' => 'blogling_sanitize_checkbox',
	) );

	$wp_customize->add_control( 'show_post_categories', array(
		'label'    => __( 'Hide post categories', 'blogling' ),
		'section'  => 'blogling_settings',
		'priority' => 1,
	...
	$wp_customize->add_setting( 'show_post_page_byline', array(
		'default' => 0,
		'sanitize_callback' => 'blogling_sanitize_checkbox',
	) );

	$wp_customize->add_control( 'show_post_page_byline', array(
		'label'    => __( 'Hide post/page author', 'blogling' ),
		'section'  => 'blogling_settings',
		'priority' => 1,
		'settings' => 'show_post_page_byline',
		'type'     => 'checkbox',
	) );



	function blogling_sanitize_checkbox( $input ) {
...
add_action( 'customize_register', 'blogling_customize_register' );




if(! function_exists('blogling_customizer_css_final_output' ) ):
	function blogling_customizer_css_final_output(){
		?>
	...
	<?php }
	add_action( 'wp_head', 'blogling_customizer_css_final_output' );
endif;